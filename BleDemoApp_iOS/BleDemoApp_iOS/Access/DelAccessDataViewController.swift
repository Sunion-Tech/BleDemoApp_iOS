//
//  DelAccessDataViewController.swift
//  BleDemoApp_iOS
//
//  Created by Cthiisway on 2024/3/19.
//

import UIKit
import SunionBluetoothTool

protocol DelAccessDataViewControllerDelegate: AnyObject {
    func delAccessData(model: DelAccessRequestModel)
}

class DelAccessDataViewController: UIViewController {
    ...
    weak var delegate: DelAccessDataViewControllerDelegate?
    ...
    @IBAction func confirmAction(_ sender: UIButton) {
        if let index = textField.text {
            ...
            var type: DelAccessRequestModel.AccessTypeMode = .AccessCode
            ...
            delegate?.delAccessData(model: DelAccessRequestModel(type: type, index:  Int(index)!))
